...
<div class="table-responsive">
            ...
            <tr>
                ...
                <td>
                    <form method="POST" action="#" style="display: inline-block;">
                        @csrf
                        <button type="button" class="btn btn-outline-secondary" data-toggle="tooltip" title='Detail' data-konf-proses="#"><i class="fa fa-chevron-circle-up"></i>Detail</button>
                    ...
                    <form method="POST" action="{{ route('user.destroy', $row->id) }}" style="display: inline-block;">
                        @method('delete')
                        @csrf
                        <button type="button" class="btn btn-outline-danger" data-toggle="tooltip" title='Hapus' data-konf-delete="{{ $row->nama }}"><i class="fa fa-trash"></i>Hapus</button>
                    </form>
                </td>
            </tr>
            @endforeach
    ...
    <div class="card-footer d-flex align-items-center">
        <p class="m-0 text-secondary"> Showing <span> {{$servis->firstItem()}} </span> to <span> {{$servis->lastItem()}} </span> of <span> {{$servis->total()}} </span> entries</p>
