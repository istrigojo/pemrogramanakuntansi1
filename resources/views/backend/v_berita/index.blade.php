...
<div class="table-responsive">
    <div class="card-body border-bottom py-3">
        <a href="{{ route('berita.create') }}" title="Tambahkan data baru">
    ...
    <table class="table card-table table-vcenter text-nowrap datatable">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td> {{$row->kategories->nama_kategori}} </td>
                <td> {{$row->tanggal}} </td>
                <td> {{$row->judul}} </td>
                <td>
    ...
    <div class="card-footer d-flex align-items-center">
        <p class="m-0 text-secondary"> Showing <span> {{$berita->firstItem()}} </span> to <span> {{$berita->lastItem()}} </span> of <span> {{$berita->total()}} </span> entries</p>
        <ul class="pagination m-0 ms-auto">
            ...
            @for ($i = 1; $i <= $berita->lastPage(); $i++)
                <li class="page-item @if($berita->currentPage() == $i) active @endif">
