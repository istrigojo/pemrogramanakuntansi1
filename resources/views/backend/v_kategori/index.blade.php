...
@section('content')
    ...
    <div class="table-responsive">
                        ...
                        <td>
                            ...
                            <form method="POST" action="{{ route('kategori.destroy', $row->id) }}"
                                ...
                                <button type="button" class="btn btn-outline-danger" data-toggle="tooltip" title='Hapus'
                                    data-konf-delete="{{ $row->kategori }}"><i
        ...
        <div class="card-footer d-flex align-items-center">
            <p class="m-0 text-secondary"> Showing <span> {{ $kategori->firstItem() }} </span> to <span>
                    {{ $kategori->lastItem() }} </span> of <span> {{ $kategori->total() }} </span> entries</p>
