...
<div class="table-responsive">
    ...
    <div class="card-footer d-flex align-items-center">
        <p class="m-0 text-secondary"> Showing <span> {{$kategories->firstItem()}} </span> to <span> {{$kategories->lastItem()}} </span> of <span> {{$kategories->total()}} </span> entries</p>
        <ul class="pagination m-0 ms-auto">
            <li class="page-item ">
                <a class="page-link" href=" {{$kategories->previousPageUrl()}} " tabindex="-1">
            ...
            @for ($i = 1; $i <= $kategories->lastPage(); $i++)
                <li class="page-item @if($kategories->currentPage() == $i) active @endif">
                    <a class="page-link" href="{{$kategories->url($i)}}"> {{$i}} </a>
                </li>
                @endfor
                <li class="page-item">
                    <a class="page-link" href="{{$kategories->nextPageUrl()}}">
