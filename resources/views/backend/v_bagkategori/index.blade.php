...
@section('content')
    ...
    <div class="table-responsive">
        ...
        <table class="table card-table table-vcenter text-nowrap datatable">
            ...
            <tbody>
                @foreach ($bagkategori as $index => $row)
                    <tr>
                        <td> {{ $bagkategori->firstItem() + $index }} </td>
                        ...
                        <td>
                            ...
                            <form method="POST" action="{{ route('bagkategori.destroy', $row->id) }}"
                                style="display: inline-block;">
                                @method('delete')
                                @csrf
                                <button type="button" class="btn btn-outline-danger" data-toggle="tooltip" title='Hapus'
                                    data-konf-delete="{{ $row->bagkategori }}"><i
        ...
        <div class="card-footer d-flex align-items-center">
            <p class="m-0 text-secondary"> Menampilkan <span> {{ $bagkategori->firstItem() }} </span> hingga <span>
                    {{ $bagkategori->lastItem() }} </span> of <span> {{ $bagkategori->total() }} </span> entries</p>
