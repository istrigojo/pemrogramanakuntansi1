...
@section('content')
    ...
    <div class="table-responsive">
                ...
                @foreach ($montir as $index => $row)
                    <tr>
                        ...
                        <td>
                            ...
                            <form method="POST" action="{{ route('montir.destroy', $row->id) }}"
                                style="display: inline-block;">
                                @method('delete')
                                @csrf
                                <button type="button" class="btn btn-outline-danger" data-toggle="tooltip" title='Hapus'
                                    data-konf-delete="{{ $row->kode_montir }}"><i
        ...
        <div class="card-footer d-flex align-items-center">
            <p class="m-0 text-secondary"> Showing <span> {{ $montir->firstItem() }} </span> to <span>
                    {{ $montir->lastItem() }} </span> of <span> {{ $montir->total() }} </span> entries</p>
